...
<section class="py-12">
    <div class="container mx-auto px-4">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('home') }}" class="text-sm text-gray-600 hover:underline">Quay về trang chủ</a>
        </div>

        @if($orders->isEmpty())
            <div class="rounded-lg bg-white p-6 shadow text-center text-gray-600">Bạn chưa có đơn hàng nào.</div>
        @else
            <div class="space-y-4">
                @foreach($orders as $order)
                    <div class="rounded-lg bg-white p-4 shadow hover:shadow-md transition-shadow duration-150 flex items-center justify-between">
                        <div>
                            <div class="text-sm text-gray-500">Đơn #{{ $order->id }} &middot; <span class="text-xs text-gray-400">{{ $order->created_at->format('Y-m-d') }}</span></div>
                            <div class="text-base font-semibold mt-1">{{ number_format($order->grand_total ?? $order->total ?? 0, 0, ',', '.') }} VNĐ</div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('orders.show', $order) }}" class="inline-flex items-center gap-2 text-main-red hover:underline">Xem chi tiết
