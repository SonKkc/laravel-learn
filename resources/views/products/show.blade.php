            ...
            <div class="grid grid-cols-1 items-start gap-10 md:grid-cols-2">
                ...
                <div class="rounded-lg bg-white p-8 shadow">
                    ...
                    <div class="mb-4 flex items-center gap-3">
                        <span class="bg-primary/10 text-primary inline-block rounded-full px-3 py-1 text-xs font-semibold">{{ $product->category->name ?? 'Category?' }}</span>
                    ...
                    <div class="mb-6 flex items-center gap-6">
                        <span class="text-sm text-gray-500">Views: <span class="font-semibold text-gray-700">{{ $product->views }}</span></span>
                        <span class="text-sm text-gray-500">Stock: <span class="font-semibold text-gray-700">{{ $product->in_stock ? 'In stock' : 'Out of stock' }}</span></span>
                        <span class="text-sm text-gray-500">Quantity: <span class="font-semibold text-gray-700">{{ $product->quantity ?? 0 }}</span></span>
                    </div>
                    <form class="flex items-center gap-4" x-data="{ qty: 1, min: 1, max: {{ $product->quantity ?? 99 }}, _added: false }">
                        <template x-if="max > 0">
                            <div class="flex flex-col gap-3 w-full">
                                <div class="flex items-center justify-between w-full">
                                    ...
                                    <span class="ml-4 text-sm text-gray-500">Stock: <span class="font-semibold text-main-red">{{ $product->quantity }}</span></span>
                                </div>
                        ...
                        <template x-if="max === 0">
                            <div class="flex flex-col gap-3 w-full opacity-60 pointer-events-none select-none">
                                <button type="button" disabled class="w-full bg-gray-300 rounded-full px-6 py-3 font-bold text-white shadow-lg text-lg tracking-wide">Out of stock</button>
                            </div>
                        </template>
                    </form>
                    <script>
                        ...
                        function showLoginPrompt() {
                            ...
                            modal.innerHTML = `
                                <div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 mx-4">
                                            ...
                                            <p class="text-sm text-gray-600 mb-4">You need to log in to add items to the cart.</p>
                                    <div class="flex gap-3 justify-end">
                                        <button id="login-prompt-cancel" class="px-4 py-2 rounded border">Cancel</button>
                                        <a id="login-prompt-login" href="${loginUrl}" class="px-4 py-2 rounded bg-main-red text-white font-semibold">Log in</a>
                                    </div>
                                </div>
                            `;
                        ...
                        document.querySelectorAll('[data-add-to-cart]').forEach(function (btn) {
                            btn.addEventListener('click', async function (e) {
                                ...
                                if (!isAuth) {
                                    ...
                                    if (window.showToast) {
                                        window.showToast('Please log in to perform this action', { type: 'info' });
                                    }
                                    showLoginPrompt();
                                    return;
                                }
